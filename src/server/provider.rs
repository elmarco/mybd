...
impl Default for BnfProvider {
    fn default() -> Self {
        Self {
            client: bnfbd::Client::new(),
        }
    }
}

impl BnfProvider {
    fn records_to_series(records: Vec<bnfbd::Record>) -> Vec<Series> {
        let mut seen = std::collections::HashMap::<String, Series>::new();

        for record in records {
            let key = Self::normalize_series_key(&record);
            seen.entry(key).or_insert_with(|| {
                let author = record
                    ...
                    .unwrap_or_default();
                let year = record.pub_date.as_deref().and_then(Self::extract_year);
                let series_title = record.series.as_deref().unwrap_or(&record.title);
                ...
                Series {
                    ...
                    year,
                    number_of_albums: None,
                    bubble_id: Some(series_title.to_string()),
                    slug: String::new(),
                    is_terminated: None,
                    created_at: String::new(),
                }
    ...
    fn normalize_series_key(record: &bnfbd::Record) -> String {
        let raw = record.series.as_deref().unwrap_or(&record.title);
        use unicode_normalization::UnicodeNormalization;
        raw.nfc().collect::<String>().trim().to_lowercase()
    }

    fn extract_year(pub_date: &str) -> Option<i32> {
        pub_date
            .chars()
            .collect::<Vec<_>>()
            .windows(4)
    ...
    fn author_to_info(a: &bnfbd::Author) -> AuthorInfo {
        let (year_of_birth, year_of_death) = a
            .dates
            .as_deref()
            .map(Self::split_dates)
            .unwrap_or((None, None));

        AuthorInfo {
            bubble_id: a.bnf_id.clone().unwrap_or_default(),
            display_name: a.display_name(),
            image_url: None,
            year_of_birth,
            year_of_death,
            external_url: None,
            source_label: Some("BnF".to_string()),
        }
    }

    fn split_dates(dates: &str) -> (Option<String>, Option<String>) {
        let mut parts = dates.splitn(2, '-');
        let birth = parts
            .next()
    ...
    fn extract_page_count(pages: &str) -> Option<i64> {
        let after_paren = pages.find('(').map(|i| &pages[i + 1..]).unwrap_or(pages);
...
impl MetadataProvider for BnfProvider {
    ...
    async fn search_series(&self, query: &str) -> Result<Vec<Series>, ServerFnError> {
        let results = self.client
            .search_by_title(query, 25)
            .await
            .map_err(|e| ServerFnError::new(e.to_string()))?;
        Ok(Self::records_to_series(results.records))
    }

    async fn search_series_all(&self, query: &str) -> Result<Vec<Series>, ServerFnError> {
        let results = self.client
            .search_by_title(query, 200)
            .await
            .map_err(|e| ServerFnError::new(e.to_string()))?;
        Ok(Self::records_to_series(results.records))
    }
    ...
    async fn search_authors(&self, query: &str) -> Result<Vec<AuthorInfo>, ServerFnError> {
        let results = self.client
            .search_by_author(query, 20)
            .await
            .map_err(|e| ServerFnError::new(e.to_string()))?;

        let mut seen = std::collections::HashSet::new();
        let authors: Vec<AuthorInfo> = results
            .records
            .iter()
            .flat_map(|r| &r.authors)
            .filter(|a| {
                let key = a.bnf_id.as_deref().unwrap_or(&a.name);
                seen.insert(key.to_string())
            })
            .take(5)
            .map(Self::author_to_info)
            .collect();
    ...
    async fn get_author_info(
        ...
        let author = results
            ...
            .iter()
            .flat_map(|r| &r.authors)
            .find(|a| a.bnf_id.as_deref() == Some(provider_id))
            .map(Self::author_to_info);
    ...
    async fn fetch_series_detail(
        ...
        let results = self.client
            .search_by_series(provider_id, 500)
            .await
            .map_err(|e| ServerFnError::new(e.to_string()))?;
        ...
        let metadata = SeriesMetadata {
            ...
            cover_url: None,
            year: first
                .and_then(|r| r.pub_date.as_deref())
                .and_then(Self::extract_year),
            number_of_albums: None,
        };

        let mut albums: Vec<AlbumMetadata> = records
            .into_iter()
            .map(|r| AlbumMetadata {
                provider_id: r.ark,
                tome: r.volume,
        ...
        albums.retain(|a| match a.tome {
            Some(v) => seen_volumes.insert(v),
            None => true,
        });
        albums.sort_by_key(|a| a.tome.unwrap_or(i32::MAX));
    ...
    async fn fetch_album_enrichment(
        ...
        let record = match self.client.get_by_ark(provider_id).await {
            Ok(Some(r)) => r,
            Ok(None) => return Ok(None),
            Err(_) => return Ok(None),
        };

        let authors: Vec<AlbumAuthor> = record
            .authors
            .iter()
            .map(|a| {
                let (date_birth, date_death) = a
                    .dates
                    .as_deref()
                    .map(Self::split_dates)
                    .unwrap_or((None, None));
        ...
        Ok(Some(AlbumEnrichment {
            summary: None,
            cover_url: None,
            publisher: record.publisher,
            number_of_pages: record.pages.as_deref().and_then(Self::extract_page_count),
